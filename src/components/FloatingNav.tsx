    ...
    <AnimatePresence>
      {pinned && (
        ...
        >
          <nav
            aria-label="Site"
            className="glass-chip flex items-center gap-2 rounded-full py-2 pl-2 pr-2 sm:gap-4 sm:pl-3"
          >
            <a
              href="#top"
            ...
            <div className="hidden items-center gap-7 px-1 text-[13px] font-medium text-[#3a352f] md:flex">
              {navLinks.map(([label, href]) => (
                <a key={label} href={href} className="transition-colors hover:text-[#14120F]">
                  {label}
                </a>
              ))}
            </div>
